...
def detect_cpg_islands(sequence, window_size):
    ...
    for i in range(0, len(sequence), window_size):
        ...
        if cg_count >= window_size // 6.66:  # 15% threshold
            cpg_count += 1
        else:
            non_cpg_count += 1
...
plt.title('CpG vs Non-CpG Islands in Chromosomes Z, W, 28 (15% Threshold)')
plt.xticks(x, chromosomes) 
plt.legend()
plt.savefig('cpg_island_comparison_15.png', dpi=300) 
plt.show()